						...
						<form class="f-12" id="order-filter-form" onsubmit="return false;" autocomplete="off">
							<p class="ml40 mb10">
								...
								<select class="select-normal"  name="state" id="state">
									...
									<option value="22">待人工审核</option>
									<option value="26">人工审核通过</option>
									<option value="27">人工审核不通过</option>
...
<script type="text/javascript">
    ...
    $(function(){
        searchtable =  $('#table_list').dataTable({
            ...
            "iDisplayLength":10,
            "ajax": {
                "url": "${path}/borrowMana/repayMortgageList.shtml"
            },
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                var a = {
                    'editTable':
                    '<a href="javascript:;" onclick=payAgain(\"'+aData.id+'\")>重新支付</a>'
                }

                if (aData.state =='22'){
                    $('td:eq(9)', nRow).html('待人工审核');
                }else if (aData.state =='27'){
                    $('td:eq(9)', nRow).html('人工审核不通过');
                }else if (aData.state =='26'){
                    $('td:eq(9)', nRow).html('人工审核通过');
                }else if (aData.state =='30'){
                    $('td:eq(9)', nRow).html('放款成功');
                }else if (aData.state =='31'){
                    $('td:eq(9)', nRow).html('<span class="font-red">放款失败</span>');
    ...
    function payAgain(id){
        layer.confirm('您确认要重新支付该订单？', {
            btn: ['确认','取消'] //按钮
        }, function(){
            $.post("${path}/payMana/payAgain.shtml",{id:id},function(result){
                if(result.success == true){
                    reloadAjax();
                    layer.msg("支付成功!");
                }else{
                    layer.msg("支付失败!"+result.msg);
                }
            });
        }, function(){
