...
<script type="text/javascript">
    ...
    $(function(){
        searchtable =  $('#table_list').dataTable({
            ...
            "iDisplayLength":10,
            "ajax": {
                "url": "${path}/riskMana/creditReviewList.shtml"
            },
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                var a = {
                    'editTable':
                    '<a onclick=audit(' + aData.id + ');>人工审核</a>'
                }

                if (aData.state =='22'){
                    $('td:eq(7)', nRow).html('待人工复审');
                    $('td:eq(9)', nRow).html(a.editTable);
                }else if (aData.state =='27'){
                    $('td:eq(7)', nRow).html('人工复审不通过');
                }else if (aData.state =='26'){
                    $('td:eq(7)', nRow).html('人工复审通过');
                }else if (aData.state =='30'){
                    $('td:eq(7)', nRow).html('放款成功');
                }else if (aData.state =='31'){
                    $('td:eq(7)', nRow).html('放款失败');
                }else if (aData.state =='40'){
                    $('td:eq(7)', nRow).html('已还款');
                }else if (aData.state =='41'){
                    $('td:eq(7)', nRow).html('减免还款');
                }else if (aData.state =='50'){
                    $('td:eq(7)', nRow).html('已逾期');
                }else{
                    $('td:eq(7)', nRow).html('坏账');
                }
    ...
    function audit(id){
        layer.open({
            ...
            resize: false,
            maxmin: false,
            content: "${path}/riskMana/creditBorrowAuditPage.shtml?id="+id
        });
    }
