						...
						<form class="f-12" id="order-filter-form" onsubmit="return false;" autocomplete="off">
							<p class="ml40">
								...
								<select class="select-normal"  name="status" id="status">
									<option value="20">待复审订单</option>
									<option value="30">复审通过</option>
...
<script type="text/javascript">
    ...
    $(function(){
        searchtable =  $('#table_list').dataTable({
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
                var a = {
                    'editTable':
                    '<a onclick=audit(\"' + aData.id + '\");>人工审核</a> | <a onclick=quota(\"' + aData.id + '\");>设置额度</a> | <a onclick=upload(\"' + aData.id + '\",\"' + aData.userId + '\");>上传用户证明</a> '
                }

                if (aData.status =='10'){
                    $('td:eq(7)', nRow).html('新申请');
                }else if (aData.status =='20'){
                    $('td:eq(7)', nRow).html('待复审');
                    $('td:eq(9)', nRow).html(a.editTable);
                }else if (aData.status =='30'){
                    $('td:eq(7)', nRow).html('通过复审');
    ...
    function quota(id){
       layer.open({
		   ...
		   resize: false,
		   maxmin: false,
		   content: "${path}/riskMana/mortgageReviewEditPage.shtml?id="+id
	   });
    }
    ...
    function upload(id,userId){
        layer.open({
            ...
            resize: false,
            maxmin: false,
            content: "${path}/riskMana/mortgageUpImagepage.shtml?cat=U&userId="+userId+"&id="+id
        });
    }
    ...
    function audit(id){
        layer.open({
            ...
            resize: false,
            maxmin: false,
            content: "${path}/riskMana/mortgageAuditPage.shtml?status=30&id="+id
        });
    }
